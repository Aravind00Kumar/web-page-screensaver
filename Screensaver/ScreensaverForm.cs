...
namespace Dashboard.Screensaver
{
    public partial class ScreensaverForm : Form
    {
        ...
        private readonly GlobalUserEventHandler userEventHandler;
        ...
        public List<string> Urls
        {
            get
            {
                if (urls == null) urls = prefsManager.CurrentPreferences.Screens[screenNum].UrlList;
                return urls;
            }
        }
        ...
        public ScreensaverForm(int? screenNumber = null)
        {
            ...
            if (screenNumber == null) screenNum = prefsManager.CurrentPreferences.Screens.FindIndex(s => s.IsPrimary);
        ...
        private void ScreensaverForm_Load(object sender, EventArgs e)
        {
            if (Urls.Any())
            {
                if (Urls.Count > 1)
                {
                    ...
                    shuffleOrder = prefsManager.CurrentPreferences.Screens[screenNum].Randomize;
                    if (shuffleOrder)
                    {
                        random = new Random();
                    ...
                    timer = new Timer
                    {
                        Interval = prefsManager.CurrentPreferences.Screens[screenNum].Interval * 1000
                    };
        ...
        private void HandleUserActivity()
        {
            ...
            if (prefsManager.CurrentPreferences.CloseOnActivity)
                Close();
            else
            {
    ...
    public class GlobalUserEventHandler : IMessageFilter
    {
        ...
        {
            if ((m.Msg == WM_MOUSEMOVE) && (lastMousePos == null))
                lastMousePos = Cursor.Position;

            if (((m.Msg == WM_MOUSEMOVE) && (Cursor.Position != lastMousePos))
